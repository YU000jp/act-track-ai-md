...
import { installDashboardRPC } from "./tauri-bridge"
...
function renderTrackingStatus(container: HTMLElement, status: TrackingStatus): void {
  container.textContent = status.running ? "Tracking: active" : "Tracking: paused";
  container.dataset.trackingState = status.state;
}
    ...
    <form id="settings-form">
      ...
      <div class="form-group">
        ...
        <div class="settings-hint">
          Status: ${settings.geminiApiKeyConfigured ? "configured" : "not configured"}. Leave blank to keep the current key.
        </div>
      </div>
      <div class="form-group">
      ...
      <h3>Tracking</h3>
      <div id="tracking-status-indicator" class="settings-feedback">Tracking: loading...</div>
...
async function hydrateFromRPC(app: HTMLElement): Promise<void> {
  ...
  const [todaySummary, topApps, settings, trackingStatus] = await Promise.all([
    rpc.getTodaySummary(),
    rpc.getTopApps(),
    rpc.getSettings(),
    rpc.getTrackingStatus(),
  ]);
  const todayStatsContainer = app.querySelector<HTMLElement>("#today-stats");
  const topAppsContainer = app.querySelector<HTMLElement>("#top-apps");
  const settingsContainer = app.querySelector<HTMLElement>("#settings-content");
  const trackingStatusContainer = app.querySelector<HTMLElement>("#tracking-status-indicator");
  ...
  if (trackingStatusContainer) {
    renderTrackingStatus(trackingStatusContainer, trackingStatus);
  }
...
function bindTrackingStatusListener(app: HTMLElement): void {
  window.addEventListener("tracking-status", (event) => {
    const detail = (event as CustomEvent<TrackingStatus>).detail;
    const container = app.querySelector<HTMLElement>("#tracking-status-indicator");
    if (container) {
      renderTrackingStatus(container, detail);
    }
  });
}
...
function bindSettingsSave(app: HTMLElement, initialSettings: AppSettings = DEFAULT_SETTINGS): void {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const geminiKeyInput = app.querySelector<HTMLInputElement>("#geminiApiKey");
    const data = new FormData(form);
    const geminiApiKey = String(data.get("geminiApiKey") ?? "").trim();
    const nextSettings: AppSettings = {
      geminiApiKeyConfigured: currentSettings.geminiApiKeyConfigured,
    ...
    const settingsToSave: AppSettingsUpdate = {
      pollIntervalMs: nextSettings.pollIntervalMs,
      idleTimeoutMs: nextSettings.idleTimeoutMs,
      notificationCooldownMs: nextSettings.notificationCooldownMs,
      gracePeriodMs: nextSettings.gracePeriodMs,
      markdownExportPath: nextSettings.markdownExportPath,
      notificationsEnabled: nextSettings.notificationsEnabled,
      autoStart: nextSettings.autoStart,
      classificationRulesJson: nextSettings.classificationRulesJson,
      summaryLanguage: nextSettings.summaryLanguage,
      summaryTone: nextSettings.summaryTone,
      markdownPrivacyMode: nextSettings.markdownPrivacyMode,
      startInBackground: nextSettings.startInBackground,
    };

    await rpc.setSettings({ settings: settingsToSave, geminiApiKey: geminiApiKey || undefined });
    if (geminiApiKey) {
      nextSettings.geminiApiKeyConfigured = true;
      if (geminiKeyInput) {
...
async function bootstrap(): Promise<void> {
  ...
  bindTrackingStatusListener(app);
